...
var libIdioCargado  			= '';
var promesaLibIdioEnCurso       = null;
var idLibIdioEnCurso            = '';
    ...
    enlaces.forEach((enlace) => { 
        if (enlace.id == 'recomendarAmigo') {
            ...
            switch (id) {
		        ...
		        case 'pt-mz':
		        case 'pt-MZ':
	                enlace.href = "mailto:?subject=Recomendação%20da%20aplicação%20My%20Contacts%20Cloud&body=Olá%2C%0D%0A%0D%0ARecomendo-lhe%20que%20experimente%20a%20aplicação%20My%20Contacts%20Cloud.%20É%20gratuita.%0D%0A%0D%0AAceda%20a%20https%3A%2F%2Fmycontacts.cloud%20para%20conhecer%20o%20My%20Contacts%20Cloud%20e%20registar-se%2C%20caso%20tenha%20interesse.%0D%0A%0D%0ACumprimentos%2C%20";
                    break;
                case 'pt-br':
                case 'pt-BR':
	                	enlace.href = "mailto:?subject=Recomenda%C3%A7%C3%A3o%20do%20aplicativo%20My%20Contacts%20Cloud&body=Ol%C3%A1%2C%0D%0A%0D%0ARecomendo%20que%20experimente%20o%20aplicativo%20My%20Contacts%20Cloud.%20Ele%20%C3%A9%20gratuito.%0D%0A%0D%0AAcesse%20https%3A%2F%2Fmycontacts.cloud%20para%20conhecer%20o%20My%20Contacts%20Cloud%20e%20se%20cadastrar%2C%20caso%20tenha%20interesse.%0D%0A%0D%0AAbra%C3%A7os%2C%20";
                    break;
                case 'fr-fr':
                case 'fr-FR':
                case 'fr-ca':
...
function iniciarEventos() {
	...
	const localeCalendarioMcc = String(idLocale || '').replace('_', '-').toLowerCase();
	const localeCalendarioMccPtEuropeu = ['pt-pt', 'pt-mz', 'pt-ao'].includes(localeCalendarioMcc);


    calendar = new FullCalendar.Calendar(calendarEl, {
		themeSystem: 'bootstrap5',
		locale: idLocale,
		noEventsContent: localeCalendarioMcc.startsWith('de')
			? 'Keine Termine vorhanden'
			: (localeCalendarioMcc.startsWith('ca')
				? 'No hi ha cap esdeveniment'
				: (localeCalendarioMcc.startsWith('gl')
					? 'Non hai ningún evento'
					: (localeCalendarioMccPtEuropeu
						? 'Não existem eventos para apresentar'
						: (localeCalendarioMcc === 'pt-br' ? 'Nenhum evento para exibir' : undefined)))),
		buttonHints: localeCalendarioMcc.startsWith('gl') ? {
			prev: 'Anterior',
			next: 'Seguinte',
			today: 'Hoxe',
			...
			next: 'Seguinte',
			today: 'Hoje',
			dayGridMonth: 'Ver o mês',
			timeGridWeek: 'Ver a semana',
			timeGridDay: 'Ver o dia',
			listWeek: 'Ver a agenda'
		} : (localeCalendarioMcc === 'pt-br' ? {
			prev: 'Anterior',
			next: 'Próximo',
			today: 'Hoje',
			dayGridMonth: 'Visualizar o mês',
			timeGridWeek: 'Visualizar a semana',
			timeGridDay: 'Visualizar o dia',
			listWeek: 'Visualizar a agenda'
		} : undefined)),
...
function libIdioma(id, n) {
	...
	var ejecutarAcciones = function(cargada) {
		if (cargada) accionesTrasCargaLibIdio(id, n);
		return cargada;
	};

	if (promesaLibIdioEnCurso && idLibIdioEnCurso == id) {
		return promesaLibIdioEnCurso.then(ejecutarAcciones);
	}

	var nomFichero = 'https://cdn.jsdelivr.net/gh/mycontactscloud/app@main/lib/idio4/mcc_' + id + '.js?v=4.0.0-27';
	var script = document.createElement('script');
	script.src = nomFichero;
	script.type = 'text/javascript';
	idLibIdioEnCurso = id;
	promesaLibIdioEnCurso = new Promise(function(resolve) {
		script.onload = function() {
			libIdioCargado = id;
			promesaLibIdioEnCurso = null;
			idLibIdioEnCurso = '';
			resolve(true);
		};
...
function mccCorregirLocaleBootstrapTablePortuguesEuropeu() {
    if (typeof $ === 'undefined' || !$.fn || !$.fn.bootstrapTable || !$.fn.bootstrapTable.locales) return;
    const localePt = $.fn.bootstrapTable.locales['pt-PT'] || $.fn.bootstrapTable.locales.pt;
    if (!localePt) return;
    localePt.formatCopyRows = () => 'Copiar linhas';
    localePt.formatPrint = () => 'Imprimir';
    localePt.formatLoadingMessage = () => 'A carregar, aguarde por favor';
    localePt.formatRecordsPerPage = (quantidade) => `${quantidade} registos por página`;
    localePt.formatShowingRows = (inicio, fim, total, semFiltrar) => {
        const text = `A mostrar os registos ${inicio} a ${fim} de ${total}`;
        return semFiltrar !== undefined && semFiltrar > total
            ? `${text} (filtrados de ${semFiltrar})`
            : text;
    };
    localePt.formatSRPaginationPreText = () => 'Página anterior';
    localePt.formatSRPaginationPageText = (pagina) => `Ir para a página ${pagina}`;
    localePt.formatSRPaginationNextText = () => 'Página seguinte';
    localePt.formatDetailPagination = (total) => `${total} ${total === 1 ? 'registo' : 'registos'}`;
    localePt.formatClearSearch = () => 'Limpar pesquisa';
    localePt.formatSearch = () => 'Pesquisar';
    localePt.formatNoMatches = () => 'Não foram encontrados registos';
    localePt.formatPaginationSwitch = () => 'Ocultar ou mostrar a paginação';
    localePt.formatPaginationSwitchDown = () => 'Mostrar a paginação';
    localePt.formatPaginationSwitchUp = () => 'Ocultar a paginação';
    localePt.formatRefresh = () => 'Atualizar';
    localePt.formatToggleOn = () => 'Mostrar a vista de cartões';
    localePt.formatToggleOff = () => 'Ocultar a vista de cartões';
    localePt.formatColumns = () => 'Colunas';
    localePt.formatColumnsToggleAll = () => 'Alternar todas';
    localePt.formatFullscreen = () => 'Ecrã inteiro';
    localePt.formatAllRows = () => 'Todos';
    localePt.formatAutoRefresh = () => 'Atualização automática';
    localePt.formatExport = () => 'Exportar dados';
    localePt.formatJumpTo = () => 'Ir';
    localePt.formatAdvancedSearch = () => 'Pesquisa avançada';
    localePt.formatAdvancedCloseButton = () => 'Fechar';
    localePt.formatFilterControlSwitch = () => 'Ocultar ou mostrar os controlos';
    localePt.formatFilterControlSwitchHide = () => 'Ocultar os controlos';
    localePt.formatFilterControlSwitchShow = () => 'Mostrar os controlos';
    $.fn.bootstrapTable.locales['pt-PT'] = localePt;
    $.fn.bootstrapTable.locales['pt-MZ'] = localePt;
    $.fn.bootstrapTable.locales['pt-AO'] = localePt;
    $.fn.bootstrapTable.locales.pt = localePt;
}
...
function mccIdiomaSummernoteContactos() {
    const locale = String(localStorage.getItem('idiomaPais') || (typeof idLocale !== 'undefined' ? idLocale : ''))
        .replace('_', '-')
        .toLowerCase();
    if (locale === 'pt-br') return 'pt-BR';
    if (['pt-pt', 'pt-mz', 'pt-ao'].includes(locale)) return 'pt-PT';
    return 'en-US';
}

async function mccCargarIdiomaSummernoteContactos() {
    const idioma = mccIdiomaSummernoteContactos();
    if (!['pt-BR', 'pt-PT'].includes(idioma)) return true;
    const fichero = idioma === 'pt-PT'
        ? 'lib/summernote/lang/summernote-pt-PT.js'
        : 'lib/summernote/lang/summernote-pt-BR.min.js';
    const url = new URL(fichero, document.baseURI);
    url.searchParams.set('v', versionMccStatic);
    return cargarLibJs({
        url: url.href,
        integrity: '',
        crossorigin: ''
...
async function libsTablas() {
    if (libsCargadas) {
        await mccCargarIdiomaSummernoteContactos();
        ...
        try {
            ...
            mccCorregirLocaleBootstrapTablePortuguesEuropeu();
...
function mccEsPortuguesBrasil(id) {
	return String(id || '').toLowerCase() === 'pt-br';
}
...
function txtFinPrueba1() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'de':
			...
			break;
		case 'pt':
			return mccEsPortuguesBrasil(id)
				? "Sua produtividade merece mais"
				: "A sua produtividade merece mais";
			break;
		case 'ca':
...
function txtFinPrueba2() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'pt':
			return mccEsPortuguesBrasil(id)
				? "Simplifique a gestão dos seus contatos, interações, documentos e eventos"
				: "Simplifique a gestão dos seus contactos, interações, documentos e eventos";
			break;
		case 'ca':
...
function fColPruebas1() {
	...
	if (d >= 1) {
		switch (id.substring(0, 2)) {
			...
			case 'pt':
				return (mccEsPortuguesBrasil(id)
					? "Em período de teste. Dias restantes: "
					: "Em período experimental. Dias restantes: ") + valorDiasRestantesHTML(d);
				break;
			case 'ca':
...
function fColPruebas3() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'pt':
			return (mccEsPortuguesBrasil(id)
				? "Prova cancelada. Dias restantes: "
				: "Período experimental cancelado. Dias restantes: ") + valorDiasRestantesHTML(d);
			break;
		case 'ca':
...
function fColPruebas4() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'pt':
			return mccEsPortuguesBrasil(id)
				? 'Teste concluído. <a href="javascript:void(0);" onclick="opPrincipal(10);">Consulte os Planos PRO</a>'
				: 'Período experimental concluído. <a href="javascript:void(0);" onclick="opPrincipal(10);">Consulte os Planos PRO</a>';
			break;
		case 'ca':
	...
	switch (id.substring(0, 2)) {
		...
		case 'pt':
			...
			if ((id == 'pt-BR') || (id == 'pt-br')) {
				...
				sug14.innerText = "pelo botão";
			} else {				
				sug12.innerText = "Importe os seus contactos de";
				sug14.innerText = "através do botão";
			}
			sug13.innerText = "e";
			sug15.innerText = "Importar";
			break;
...
function txtPruebas5() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'de':
			...
			break;
		case 'pt':
			return mccEsPortuguesBrasil(id) ? "Prova ativada" : "Período experimental ativado";
			break;
		case 'ca':
...
function txtPagosPendientes() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'pt':
			return mccEsPortuguesBrasil(id)
				? "Resolva os pagamentos pendentes (botão 'Assinatura')"
				: "Resolva os pagamentos pendentes (botão 'Subscrição')";
			break;
		case 'ca':
...
function txtSuscripcionNuevoPlan() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'pt':
			return mccEsPortuguesBrasil(id)
				? "Assine um novo plano (botão 'Planos PRO')"
				: "Subscreva um novo plano (botão 'Planos PRO')";
			break;
		case 'ca':
...
function txtCambiarPlan() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'de':
			...
			break;
		case 'pt':
			return mccEsPortuguesBrasil(id) ? "Mudar" : "Alterar";
			break;
		case 'ca':
			return "Canviar";
			break;
...
function txtPlanCambiado() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'de':
			...
			break;
		case 'pt':
			return mccEsPortuguesBrasil(id) ? "Plano mudado" : "Plano alterado";
			break;
		case 'ca':
			return "Pla canviat";
			break;
...
function txtCambiarPlanAhora() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'de':
			...
			break;
		case 'pt':
			return mccEsPortuguesBrasil(id) ? "Mudar de plano" : "Alterar plano";
			break;
		case 'ca':
			return "Canviar pla";
			break;
...
function autorizacionContactosGoogleMicrosoft(t) {
  ...
  switch (id.substring(0, 2)) {
    ...
    case 'pt':
      return mccEsPortuguesBrasil(id)
        ? "O My Contacts Cloud solicita sua permissão para ver e baixar seus contatos do " + t + ". Exibiremos os dados em uma tabela para que você possa selecionar os contatos que deseja importar."
        : "O My Contacts Cloud solicita a sua autorização para ver e descarregar os seus contactos do " + t + ". Os dados serão apresentados numa tabela para que possa selecionar os contactos que pretende importar.";
    case 'ca':
...
function txtContrasenaIncorrecta() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'de':
			...
			break;
		case 'pt':
			return mccEsPortuguesBrasil(id) ? "Senha incorreta" : "Palavra-passe incorreta";
			break;
		case 'ca':
...
function opcionesSummernoteContactos() {
	...
	return {
		lang: mccIdiomaSummernoteContactos(),
		toolbar: [
			['style', ['style']],
...
function fTextPromocion(n) {
    ...
    switch (id.substring(0, 2)) {
        ...
        case 'pt':
            ...
            switch (n) {
                case 1: return '🎁 Presente de boas-vindas 🎁';
                case 2: return '4 meses gratuitos nos planos anuais!';
                case 3: return 'Disponível apenas durante o seu período experimental gratuito de 10 dias';
                case 4: return "Não perca a oportunidade antes do fim do período experimental (4 meses gratuitos em vez de 2 no primeiro ano)!";
                default: return '';
            }
        case 'ca':
            switch (n) {
...
function textoCerrarSesionAlSalirMcc(id = "") {
    const locale = String(id || localStorage.getItem("idiomaPais") || navigator.language || "es")
        .replace("_", "-")
        .toLowerCase();
    const idioma = locale.substring(0, 2);

    switch (idioma) {
        case "en": return "Log out on exit";
        case "fr": return "Se déconnecter en quittant";
        case "it": return "Disconnetti alla chiusura";
        case "pt": return locale === "pt-br" ? "Sair ao fechar" : "Terminar sessão ao sair";
