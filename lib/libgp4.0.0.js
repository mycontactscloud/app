...
function cambiarAltoTabla(tablaId, padding) {
    ...
    //setTimeout(() => {
        ...
        var alturaFila = 0;
        if (tablaId === "tabContactos") 			{ alturaFila = $("#filaContactos").height(); }
        if (tablaId === "tabImportGoogle") 			{ alturaFila = $("#filaGoogle").height(); }
        if (tablaId === "tabImportMs") 				{ alturaFila = $("#filaMs").height(); } 
        if (tablaId === "tabImportApple") 			{ alturaFila = $("#filaApple").height(); }
        if (tablaId === "tabImportAppleDevice") 	{ alturaFila = $("#filaAppleDevice").height(); }
        if (tablaId === "tabDocumentos") 			{ alturaFila = $("#filaDocumentos").height(); }
        if (tablaId === "tabInteracciones") 		{ alturaFila = $("#filaInteracciones").height(); }
        if (tablaId === "tabRecordatorios") 		{ alturaFila = $("#filaRecordatorios").height(); }
        ...
        $("#" + tablaId).bootstrapTable("resetView", { height: alturaTabla });
...
function crearToastDocumento(t, m, o) {
    ...
    h +=    '               <button type="button" class="btn-close btn-sm" data-bs-dismiss="toast" aria-label="Close" title="Cerrar" data-bs-toggle="tooltip"></button>';
    h +=    '              </div>';
    h +=    '          </div>';
    h +=    '      </div>';
    h +=    '</div>';
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content rounded-4 shadow">
                    <div class="modal-header bg-light sticky-top">
                        ...
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="saveColumnConfig('${idTabla}')"></button>
                    </div>
...
function modMsjFinPruebaToast(n, t, m, o) {
    ...
    h +=    '       <button type="button" class="btn-close" id="close-toast" aria-label="Close"></button>'; // Botón de cierre manual
    h +=    '   </div>';
    h +=    '   <div class="toast-body">';
...
var mccExpComplementarias = {
    ...
    "Esta operación corresponde exclusivamente al propietario.": {ca:"Aquesta operació correspon exclusivament al propietari.",eu:"Eragiketa hau jabeari dagokio soilik.",gl:"Esta operación corresponde exclusivamente ao propietario.",en:"Only the owner can perform this operation.",it:"Questa operazione può essere eseguita solo dal proprietario.",pt:"Esta operação só pode ser efetuada pelo proprietário.",fr:"Seul le propriétaire peut effectuer cette opération.",de:"Dieser Vorgang kann nur vom Eigentümer ausgeführt werden."},
...
function cambiarTextoIdioma(idPa) {
    const spans     = document.querySelectorAll('span', 'option[span]');
    spans.forEach((span) => { span.innerHTML = tExp(span.innerHTML, idPa); });
    ...
    tooltips.forEach((tooltip) => {
        tooltip.title = tExp(tooltip.title, idPa);
        const b = new bootstrap.Tooltip(tooltip);
    });    
...
function cambiarTextoIdiomaDiv(idPa, d) {
    ...
    const spans     = divActivo.querySelectorAll('span', 'option[span]');
    spans.forEach((span) => { span.innerHTML = tExp(span.innerHTML, idPa); });

    const inputs    = divActivo.querySelectorAll('input[placeholder], textarea[placeholder]');
    inputs.forEach((input) => { input.placeholder = tExp(input.placeholder, idPa); });

    const tooltips 	= divActivo.querySelectorAll('[data-bs-toggle="tooltip"]');
    tooltips.forEach((tooltip) => {
        tooltip.title 	= tExp(tooltip.title, idPa);
        const b 		= new bootstrap.Tooltip(tooltip);
    });    
    ...
    enlaces.forEach((enlace) => { 
        if (enlace.id == 'recomendarAmigo') {
            ...
            switch (id) {
		        ...
		        case 'it-ch':
		        case 'it-CH':                	
                	enlace.href = "mailto:?subject=Raccomandazione%20dell'app%20My%20Contacts%20Cloud&body=Ciao%2C%0D%0A%0D%0Aconsiglio%20di%20provare%20l'app%20My%20Contacts%20Cloud.%20È%20gratuita%20e%20per%20sempre.%0D%0A%0D%0ACliccando%20su%20https://mycontacts.cloud%20potrai%20accedere%20al%20sito%20web%20di%20My%20Contacts%20e%20puoi%20iscriverti%20se%20sei%20interessato.%0D%0A%0D%0ASaluti%20";
                    break;
                case 'pt-pt':
                case 'pt-PT':
		        case 'pt-ao':
...
async function importHTML(s) {
    try {
        //const response = await fetch(`${s}.html`);
        var response = '';
		if ((s == 'importarappledevice') || (s == 'contactos') || (s == 'documentos')) {
        	response = await fetch('https://' + window.location.host + '/app/' + s + '.html?v=' + versionMccStatic, { cache: 'no-store' });
        } else {
        	response = await fetch(`https://mycontactscloud.github.io/app/${s}.html?v=${versionMccStatic}`, { cache: 'no-store' });	
        }
...
function iniciarContactos() {
	...
	setTimeout(function() {
	    ...
	    $("#tabContactos").bootstrapTable({
            ...
            onSort: function (name, order) {
                setTimeout(function() {
                	...
                	if ($("#tabContactos").bootstrapTable('getOptions').pageNumber != 1) {
                		...
                		selRowTab('tabContactos', 'success', 'idTabContactos', $('#tabContactos').bootstrapTable('getData')[0].ccn);
                	}
                }, 500); 
            },
            ...
            onPageChange: function (number, size) {
                setTimeout(function() {
	                ...
	                selRowTab('tabContactos', 'success', 'idTabContactos', $('#tabContactos').bootstrapTable('getData')[indiceFilaTab].ccn);
	            }, 500);
            },      
		    onPostBody: function () {
		        ...
		        if (vistaAntTabContactos !== vistaActTabContactos) {
		            ...
		            setTimeout(function () {
		                selRowTab('tabContactos', 'success', 'idTabContactos', window.idTabContactos);
		            }, 200);
		        }
		    },
        ...
        if (o.slice(0, 9) == 'landscape') {
            pCompleta_contactos.style.display = '';
        } else {
            pCompleta_contactos.style.display = 'none';
...
function iniciarDocumentos() {
    ...
    $("#tabDocumentos").bootstrapTable({
        ...
        onSort: function (name, order) {
			setTimeout(function() {
				...
				selRowTab('tabDocumentos', 'success1', 'idTabDocumentos', $('#tabDocumentos').bootstrapTable('getData')[0].id);
			}, 500); 
        },
        onSearch: function (text) {
        	selRowTab('tabDocumentos', 'success1', 'idTabDocumentos', $('#tabDocumentos').bootstrapTable('getData')[0].id);
    ...
    $("#tabDocumentos").on("load-success.bs.table", function () {
    ...
    $("#tabParticipantes").bootstrapTable({
        ...
        onSort: function (name, order) {
			setTimeout(function() {
				...
				selRowTab('tabParticipantes', 'success2', 'idTabParticipantes', $('#tabParticipantes').bootstrapTable('getData')[0].ccn);
			}, 500); 
        },
        onSearch: function (text) {
        	selRowTab('tabParticipantes', 'success2', 'idTabParticipantes', $('#tabParticipantes').bootstrapTable('getData')[0].ccn);
        ...
        onPostBody: function () {
		    ...
		    if (vistaAntTabParticipantes !== vistaActTabParticipantes) {
				...
				setTimeout(function() {
					selRowTab('tabParticipantes', 'success2', 'idTabParticipantes', window.idTabParticipantes);
				}, 200);
		    }
        }, 
	    onLoadSuccess: function () {
...
function libIdioma(id, n) {
	if (libIdioCargado == id) {
	    ...
	    var nomFichero    	= 'https://cdn.jsdelivr.net/gh/mycontactscloud/app/lib/idio4/mcc_' + id + '.js';
...
function txtFinPrueba2() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'it':
			...
			break;
		case 'de':
			return "Vereinfachen Sie die Verwaltung Ihrer Kontakte, Interaktionen, Dokumente und Veranstaltungen";
			break;
		case 'pt':
	...
	if (d >= 1) {
		switch (id.substring(0, 2)) {
			...
			case 'it':
				...
				break;
			case 'de':
				return "Im Testzeitraum. Verbleibende Tage: " + valorDiasRestantesHTML(d);
				break;
			case 'pt':
...
function fColPruebas3() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'it':
			...
			break;
		case 'de':
			return "Probe abgebrochen. Verbleibende Tage: " + valorDiasRestantesHTML(d);
			break;
		case 'pt':
...
function txtSug1() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'it':
			...
			sug13.innerText = "e";
			sug14.innerText = "cliccando su";
			sug15.innerText = "Importa";
			break;
		case 'de':
			sug11.innerText = "Vorschlag";
			sug12.innerText = "Importiere deine Kontakte aus";
			sug13.innerText = "und";
			sug14.innerText = "indem Sie auf klicken";
		...
		case 'gl':
			...
			sug13.innerText = "e";
			sug14.innerText = "clicando sobre";
			sug15.innerText = "Importar";
			break; 
		default: // en
			sug11.innerText = "Suggestion";
...
function txtSuscripcionNuevoPlan() {
	...
	switch (id.substring(0, 2)) {
		...
		case 'it':
			...
			break;
		case 'de':
			return "Abonnieren Sie einen neuen Plan (Schaltfläche 'PRO-Pläne')";
			break;
		case 'pt':
...
function autorizacionContactosGoogleMicrosoft(t) {
  ...
  switch (id.substring(0, 2)) {
    ...
    case 'it':
      return "My Contacts Cloud richiede il tuo permesso per visualizzare e scaricare i tuoi contatti " + t + ". Visualizzeremo i dati in una tabella in modo che tu possa selezionare i contatti che desideri importare.";
    case 'de':
      return "My Contacts Cloud bittet um Ihre Erlaubnis, Ihre " + t + "-Kontakte einzusehen und herunterzuladen. Wir werden die Daten in einer Tabelle anzeigen, damit Sie die Kontakte auswählen können, die Sie importieren möchten.";
    case 'pt':
    ...
    $("#tabInteracciones").bootstrapTable({
        ...
        onSort: function (name, order) {
            setTimeout(function() {
                ...
                selRowTab('tabInteracciones', 'success3', 'idTabInteracciones', $('#tabInteracciones').bootstrapTable('getData')[0].idii);
            }, 500); 
        },
        onSearch: function (text) {
            selRowTab('tabInteracciones', 'success3', 'idTabInteracciones', $('#tabInteracciones').bootstrapTable('getData')[0].idii);
        ...
        onPostBody: function () {
            ...
            if (vistaAntTabInteracciones !== vistaActTabInteracciones) {
                ...
                setTimeout(function() {
                    selRowTab('tabInteracciones', 'success3', 'idTabInteracciones', window.idTabInteracciones);
                }, 200);                
            }
        },
        onLoadSuccess: function () {
...
function iniciarRecordatorios() {
    ...
    $("#tabRecordatorios").bootstrapTable({
        ...
        onSort: function (name, order) {
            setTimeout(function() {
                selRowTab('tabRecordatorios', 'success4', 'idTabRecordatorios', $('#tabRecordatorios').bootstrapTable('getData')[0].idir);
            }, 500); 
        },
        onSearch: function (text) {
            selRowTab('tabRecordatorios', 'success4', 'idTabRecordatorios', $('#tabRecordatorios').bootstrapTable('getData')[0].idir);
        ...
        onPostBody: function () {
            ...
            if (vistaAntTabRecordatorios !== vistaActTabRecordatorios) {
                ...
                setTimeout(function() {
                    selRowTab('tabRecordatorios', 'success4', 'idTabRecordatorios', window.idTabRecordatorios);
                }, 200);                
            }
        },
        onLoadSuccess: function () {
    ...
    switch (id.substring(0, 2)) {
        ...
        case 'de':
            switch (n) {
                ...
                case 2: return '4 Monate gratis bei Jahresplänen!'; 
                case 3: return 'Nur verfügbar während deiner 10-tägigen kostenlosen Testversion'; 
                case 4: return "Verpasse nicht die Gelegenheit, bevor deine Testphase endet (4 Monate kostenlos statt 2 im ersten Jahr)!"; 
                default: return '';
            }
        case 'pt':
            switch (n) {
...
function textoCerrarSesionAlSalirMcc(id = "") {
    ...
    switch (idioma) {
        ...
        case "fr": return "Déconnexion à la sortie";
        case "it": return "Disconnetti all'uscita";
...
function abrirPlanesProveedor() {
    if (!mccEsPropietario()) {
        modal1('My Contacts Cloud', 'La gestión de planes corresponde al propietario del equipo.', msjModPrincipal);
        return;
    }
    ...
    if (sistema === 1) {
        ...
        modal1('My Contacts Cloud', 'El sistema de pagos no está configurado correctamente.', msjModPrincipal);
    }
}

function abrirGestionSuscripcion() {
    if (!mccEsPropietario()) {
        modal1('My Contacts Cloud', 'La gestión de la suscripción corresponde al propietario del equipo.', msjModPrincipal);
        return;
    }
    const sistema = mccSistemaPagos();
    if (sistema === 1) {
...
async function abrirPortalPaddle() {
    try {
        ...
        modal1('My Contacts Cloud', error.message, msjModPrincipal);
    }
}
...
function mccFormatoBytes(bytes) {
    ...
    if (valor < 1000000) return (valor / 1000).toFixed(1) + ' KB';
    if (valor < 1000000000) return (valor / 1000000).toFixed(1) + ' MB';
    return (valor / 1000000000).toFixed(2) + ' GB';
}
...
function mccPintaUso(idTexto, idBarra, usado, limite, formateador) {
    ...
    if (texto) texto.textContent = formateador(usado) + (limite > 0 ? ' de ' + formateador(limite) : '');
    if (barra) {
...
async function iniciarDashboard() {
    try {
        ...
        if (!data.success) throw new Error(data.message || 'No se pudo cargar el resumen');
        const r = data.data;
		const version = document.getElementById('dashVersion');
		if (version) version.textContent = r.version || '4.0.0';
        document.getElementById('dashBienvenida').textContent = 'Hola, ' + (r.nombre || '');
        document.getElementById('dashPlan').textContent = r.planNombre || 'Plan pendiente de identificar';
        document.getElementById('dashEstado').textContent = r.estadoSuscripcion || 'Sin suscripción';
        ...
        if (pago) {
            pago.textContent = r.proveedor ? 'Facturación: ' + r.proveedor : 'La facturación la gestiona el propietario';
        }
    } catch (error) {
...
async function mccCargarEquipo() {
    ...
    if (!data.success) throw new Error(data.message || 'No se pudo cargar el equipo');
    const r = data.data;
    document.getElementById('equipoUso').textContent = r.usuariosUtilizados + ' de ' + r.usuariosIncluidos + ' usuarios utilizados';
    ...
    r.miembros.forEach((m) => {
        ...
        [m.nombre || '—', m.email, m.rol, m.estado].forEach((valor) => {
...
function mccBotonEquipo(texto, operacion, miembro) {
    ...
    boton.addEventListener('click', async () => {
        ...
        if (operacion === 'retirar' || operacion === 'cancelar') {
            mostrarModal('¿Confirmas esta acción?', ejecutar);
        } else {
            await ejecutar();
        }
    });
    ...
    if (form && !form.dataset.preparado) {
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (!data.success) throw new Error(data.message || 'No se pudo enviar la invitación');
                form.reset();
                await mccCargarEquipo();
...
function mccMostrarMensajeEquipo(mensaje, esError) {
	...
	modal1('My Contacts Cloud', mensaje, host);
}
