...
function libIdioma(id, n) {
	if (libIdioCargado == id) {
	    ...
	    var nomFichero    	= 'https://cdn.jsdelivr.net/gh/mycontactscloud/app/lib/idio4/mcc_' + id + '.js?v=4.0.0-18';
...
function mccCorregirLocaleBootstrapTableGalego() {
    if (typeof $ === 'undefined' || !$.fn || !$.fn.bootstrapTable || !$.fn.bootstrapTable.locales) return;
    let localeGl = $.fn.bootstrapTable.locales['gl-ES'] || $.fn.bootstrapTable.locales.gl;
    if (!localeGl) {
        localeGl = Object.assign({}, $.fn.bootstrapTable.locales['es-ES'] || {});
        $.fn.bootstrapTable.locales['gl-ES'] = localeGl;
    }
    localeGl.formatShowingRows = (inicio, fin, total, senFiltrar) => {
        const text = `Filas ${inicio} a ${fin} de ${total}`;
        return senFiltrar !== undefined && senFiltrar > total
            ? `${text} (filtradas de ${senFiltrar})`
            : text;
    };
    localeGl.formatDetailPagination = (total) => `${total} ${total === 1 ? 'fila' : 'filas'}`;
    localeGl.formatRecordsPerPage = (cantidade) => `${cantidade} filas por páxina`;
    localeGl.formatClearSearch = () => 'Limpar o filtro';
}
